...
The last step was publishing it to GitHub Pages.
That was where the local setup became public: the site had to be pushed, built, and served through the Pages workflow instead of staying only in Docker.
The first attempt exposed an auth issue, so we had to get the token scope right before the deploy would go through.

Once that was fixed, the page went live and I could check the public version from the same conversation.
That fit the rest of the process: keep the setup local until it works, then publish it once the path is clear.
